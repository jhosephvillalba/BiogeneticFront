...
const ReportDetails = () => {
  ...
  // Calcular totales
  const total_gi = registros.reduce((sum, reg) => sum + (parseInt(reg.gi) || 0), 0);
  const total_gii = registros.reduce((sum, reg) => sum + (parseInt(reg.gii) || 0), 0);
  const total_giii = registros.reduce((sum, reg) => sum + (parseInt(reg.giii) || 0), 0);
  const total_viables = registros.reduce((sum, reg) => sum + (parseInt(reg.viables) || 0), 0);
  const total_otros = registros.reduce((sum, reg) => sum + (parseInt(reg.otros) || 0), 0);
  const total_total_oocitos = registros.reduce((sum, reg) => sum + (parseInt(reg.total_oocitos) || 0), 0);
  const total_ctv = registros.reduce((sum, reg) => sum + (parseInt(reg.ctv) || 0), 0);
  const total_clivados = registros.reduce((sum, reg) => sum + (parseInt(reg.clivados) || 0), 0);
  const total_prevision = registros.reduce((sum, reg) => sum + (parseInt(reg.prevision) || 0), 0);
  const total_empaque = registros.reduce((sum, reg) => sum + (parseInt(reg.empaque) || 0), 0);
  const total_vt_dt = registros.reduce((sum, reg) => sum + (parseInt(reg.vt_dt) || 0), 0);
  const total_total = total_prevision + total_empaque + total_vt_dt;
  const porcentaje_final = total_ctv > 0 ? Math.round((total_total / total_ctv) * 100) : 0;
  ...
  const embrionesData = donantes.map(donante => {
    const reg = registros.find(r => r.donante_nombre === donante);
    if (!reg) return 0;
    const total = (parseFloat(reg.prevision) || 0) + (parseFloat(reg.empaque) || 0) + (parseFloat(reg.vt_dt) || 0);
      ...
      <div className="detailed-report" ref={reportRef}>
        ...
        {muestraInfo && (
          <div className="alert alert-info mt-3">
            <h5 className="mb-2"><i className="bi bi-droplet me-2"></i>Información de la muestra utilizada</h5>
            <ul className="mb-0">
              <li><b>Toro:</b> {muestraInfo.toro.name} (Registro: {muestraInfo.toro.register})</li>
              <li><b>Raza:</b> {muestraInfo.toro.razaNombre || muestraInfo.toro.race_name || muestraInfo.toro.race_id}</li>
              <li><b>Cantidad utilizada:</b> {muestraInfo.output.quantity_output}</li>
              <li><b>Cantidad recibida:</b> {muestraInfo.input.quantity_received}</li>
              <li><b>Cantidad disponible:</b> {muestraInfo.input.total}</li>
              <li><b>Fecha de la muestra:</b> {muestraInfo.input.created_at ? new Date(muestraInfo.input.created_at).toLocaleDateString('es-CO') : 'N/A'}</li>
              {muestraInfo.output.remark && <li><b>Observaciones:</b> {muestraInfo.output.remark}</li>}
            </ul>
          </div>
        )}
